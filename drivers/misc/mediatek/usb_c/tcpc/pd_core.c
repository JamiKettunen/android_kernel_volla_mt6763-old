	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		pr_debug("%s fix_bat_info[%d].mfrs_info.vid = 0x%x, \
					    .mfrs_info.pid = 0x%x, \
					    .mfrs_string = %s, \
					    .bat_design_cap = %d\n",
			__func__, i,
...
{
	pr_debug("%s vid = 0x%x, pid = 0x%x, xid = 0x%x, \
		fw_ver = 0x%x, hw_ver = 0x%0x\n",
		__func__,
		cap->vid, cap->pid, cap->xid,
		cap->fw_ver, cap->hw_ver);
	pr_debug("%s voltage_regulation = %d, hold_time_ms = %d, compliance = 0x%x, \
	...
	pr_debug("%s touch_temp = %d, source_inputs = 0x%x, \
...
static int pd_parse_pdata(struct pd_port *pd_port)
{
	...
	if (np) {
		...
		if (of_property_read_u32(np, "pd,charging_policy", &val) < 0)
			pr_err("%s get charging policy fail\n", __func__);
